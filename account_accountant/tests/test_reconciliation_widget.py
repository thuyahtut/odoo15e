...
class TestReconciliationWidget(TestAccountReconciliationCommon):
    ...
    def test_payment_matching_order(self):
        ...
        inv_a = self.create_invoice(invoice_amount=100)
        inv_b = self.create_invoice(invoice_amount=300)
        inv_c = self.create_invoice(invoice_amount=200)

        payment = self.env['account.payment'].create({
            'payment_method_line_id': self.inbound_payment_method_line.id,
            'payment_type': 'inbound',
            'partner_type': 'customer',
            'partner_id': inv_a.partner_id.id,
            'amount': 500,
            'journal_id': self.bank_journal_euro.id,
            'company_id': self.company.id,
        })
        ...
        receivable_line_ids = []
        for m in (payment, inv_a, inv_b, inv_c):
            receivable_line_ids.append(m.line_ids.filtered(lambda l: l.account_id.internal_type == 'receivable').id)
        ...
        self.assertEqual(inv_a.payment_state, 'in_payment')
        self.assertEqual(inv_b.payment_state, 'in_payment')
        self.assertEqual(inv_c.payment_state, 'partial')
