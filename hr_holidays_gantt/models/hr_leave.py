...
class HrLeave(models.Model):
    ...
    def _group_leaves(self, leaves, employee_id, date_from, date_to):
        ...
        def show_hours(start_dt, end_dt):
        ...
        for leave in leaves:
            ...
            if not periods or has_working_hours(periods[-1]['from'], leave.date_to):
                periods.append({'is_validated': is_validated, 'from': leave.date_from, 'to': leave.date_to, 'show_hours': show_hours(leave.date_from, leave.date_to)})
            else:
                periods[-1]['is_validated'] = is_validated
                periods[-1]['to'] = leave.date_to if leave.date_to > periods[-1]['to'] else periods[-1]['to']
                periods[-1]['show_hours'] = show_hours(periods[-1]['from'], periods[-1]['to'])
        return periods
