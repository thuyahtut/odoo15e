...
class MailChannel(models.Model):
    ...
    def execute_command_helpdesk(self, **kwargs):
        ...
        channel_partners = self.with_context(active_test=False).channel_partner_ids.filtered(lambda partner: partner != self.env.user.partner_id)
        if key[0].lower() == '/helpdesk':
            if len(key) == 1:
                if self.channel_type == 'channel':
                    ...
                    channel_partners = channel_partners or self.env.user.partner_id
